...
class Switching extends Controller{
	...
	function index(){
		...
		$data['page'] = 1;
		$_count=0;
		$this->model = new Switching_Model();
		$this->view->selectdatatable = $this->model->selectdatatable($data);
		$this->view->css =array('switching-content-table.css');
		$this->view->render('switching/index');
	}
	function select(){
		$data = array();
		$kkk = array();
	...
	function switched($id){
			...
			$this->view->css =array('switching-content.css');
			//$this->view->js =array('jquery.js','group-add.js');
			$this->view->render('switching/add');
	}
	function create($id){
		$data = array();
		$this->model   = new Switching_Model();
		$data['date']        =  $this->model->secure($_POST['date']);
		$data['oldaddr']     =  $this->model->secure($_POST['oldaddr']); 
		$data['newaddr']     =  $this->model->secure($_POST['newaddr']); 
		$data['note']        =  $this->model->secure($_POST['note']);
		$data['addrname']        =  $this->model->secure($_POST['addrname']);
		$d=$id;
